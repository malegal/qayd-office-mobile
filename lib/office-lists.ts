...
export async function getCaseDetails(officeId: string, caseId: string, includeFinance = true): Promise<CaseDetails> {
  ...
  const [sessions, feesRows, payments, transactions] = await Promise.all([
    ...
    includeFinance ? getFees(officeId, [caseId]) : Promise.resolve([]),
    includeFinance ? getPayments(officeId, [caseId]) : Promise.resolve([]),
    includeFinance ? getFinancialTransactions(officeId).then((rows) => rows.filter((row) => row.case_id === caseId)) : Promise.resolve([]),
  ]);
