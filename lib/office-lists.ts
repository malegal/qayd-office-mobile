...
export type ExpenseRow = { id: string; case_id: string | null; amount: number; expense_date: string; category: string; description: string | null };
...
async function list<T>(table: string, officeId: string, select: string, cacheKey: string, configure?: (query: any) => any): Promise<T[]> {
  try {
    let query = supabase.from(table).select(select).limit(100);
    query = configure ? configure(query) : query.eq("office_id", officeId);
    const { data, error } = await query;
    if (error) throw error;
    const rows = (data ?? []) as T[];
    await cacheJson(`${cacheKey}:${officeId}`, rows);
    return rows;
  } catch (error) {
    const cached = await readCachedJson<T[]>(`${cacheKey}:${officeId}`);
...
export const getExpenses = (officeId: string) => list<ExpenseRow>("expenses", officeId, "id, case_id, amount, expense_date, category, description", "expenses", (q) => q.eq("office_id", officeId).order("expense_date", { ascending: false }));
