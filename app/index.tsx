...
export default function LoginScreen() {
  ...
  const [mode, setMode] = useState<"login" | "signup" | "join">("login"); const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [officeName, setOfficeName] = useState(""); const [inviteCode, setInviteCode] = useState(""); const [displayName, setDisplayName] = useState(""); const [joinAccountCreated, setJoinAccountCreated] = useState(false); const [submitting, setSubmitting] = useState(false); const [error, setError] = useState("");
  const translateAuthError = (message: string) => message.toLowerCase().includes("invalid login credentials") ? "البريد الإلكتروني أو كلمة المرور غير صحيحة. استخدم كلمة المرور التي أنشأتها لهذا الحساب." : message.toLowerCase().includes("email not confirmed") ? "يجب تأكيد البريد الإلكتروني أولًا من الرسالة المرسلة إليك." : message.toLowerCase().includes("user already registered") ? "هذا الحساب موجود بالفعل. استخدم كلمة المرور المسجلة ثم اضغط دخول وقبول الدعوة." : message;
  ...
  const submit = async () => {
    if (!email.trim() || !password || (mode === "signup" && !officeName.trim()) || (mode === "join" && (!inviteCode.trim() || !displayName.trim()))) {
      return setError(mode === "signup" ? "أدخل البريد وكلمة المرور واسم المكتب." : mode === "join" ? "أدخل البريد أو رقم الهاتف وكلمة المرور الجديدة وكود الدعوة والاسم." : "أدخل البريد وكلمة المرور.");
    }
      ...
      if (mode === "login") {
        ...
        if (existing.error) {
          ...
          if (!created.data.session) {
            setJoinAccountCreated(true);
            setError(isPhone ? "تم إنشاء الحساب. تحقق من رسالة الهاتف ثم اضغط الزر مرة أخرى لقبول الدعوة." : "تم إنشاء الحساب. تحقق من البريد ثم اضغط الزر مرة أخرى لقبول الدعوة.");
            return;
          }
        }
        await acceptInvite(inviteCode.trim(), displayName.trim());
      } else {
